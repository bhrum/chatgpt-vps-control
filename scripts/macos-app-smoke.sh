...
python3 -c 'import json,sys; data=json.load(sys.stdin); apps=data.get("applications", []); assert data["ok"] is True; assert isinstance(apps, list); assert all({"id", "displayName", "path", "isRunning"} <= set(app) for app in apps)' <<<"$APPLICATIONS"
