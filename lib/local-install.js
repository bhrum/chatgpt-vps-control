...
function validEnvValue(value, name) {
  const normalized = String(value ?? "");
  if (!normalized || /[\r\n\0]/u.test(normalized)) throw new Error(`${name} is invalid.`);
  return normalized;
}

export async function configureDeviceGateway({ gatewayUrl, gatewayToken, deviceId, deviceName, ipFamily = "" }) {
  const values = {
    DEVICE_GATEWAY_URL: validEnvValue(gatewayUrl, "gatewayUrl"),
    DEVICE_GATEWAY_TOKEN: validEnvValue(gatewayToken, "gatewayToken"),
    DEVICE_ID: validEnvValue(deviceId, "deviceId"),
    DEVICE_NAME: validEnvValue(deviceName, "deviceName"),
  };
  if (!/^wss:\/\//u.test(values.DEVICE_GATEWAY_URL)) throw new Error("The device gateway URL must use wss://.");
  if (values.DEVICE_GATEWAY_TOKEN.length < 32) throw new Error("The device gateway credential is invalid.");
  if (!/^[a-zA-Z0-9._-]{1,128}$/u.test(values.DEVICE_ID)) throw new Error("The device ID is invalid.");
  if (!/^[\p{L}\p{N} ._()-]{1,200}$/u.test(values.DEVICE_NAME)) throw new Error("The device name is invalid.");
  if (ipFamily) {
    if (!["4", "6"].includes(String(ipFamily))) throw new Error("ipFamily must be 4 or 6.");
    values.DEVICE_GATEWAY_IP_FAMILY = String(ipFamily);
  }
  const current = await readFile(envPath, "utf8");
