...
async function ensureLinuxDependencies() {
  const required = ["xdotool", "ffmpeg", "xrandr", "xdpyinfo", "xmodmap"];
  ...
  const packages = ["xdotool", "ffmpeg", "x11-utils", "x11-xserver-utils", "xvfb", "x11vnc", "xfwm4"];
  ...
  return { installed: packages, missing: required.filter((name) => !commandExists(name)) };
}
...
export async function doctorLocalComputer() {
  ...
  if (platform() === "linux") {
    for (const binary of ["xdotool", "ffmpeg", "xrandr", "xdpyinfo", "xmodmap"]) push(binary, commandExists(binary));
