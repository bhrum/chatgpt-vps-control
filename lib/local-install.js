...
async function ensureMacHelper(codesignIdentity = "", teamIdentifier = "") {
  ...
  const [sourceBytes, plistBytes] = await Promise.all([readFile(source), readFile(plistSource)]);
  const buildId = createHash("sha256").update(sourceBytes).update(plistBytes).update(identity).update(teamIdentifier).digest("hex");
  try {
    ...
    if (installedBuildId === buildId && signature.status === 0 && await canExecute(macHelperExecutable)) return macHelperExecutable;
  } catch {}
  ...
  const signArgs = ["--force", "--deep", "--sign", identity, "--identifier", macBundleIdentifier];
