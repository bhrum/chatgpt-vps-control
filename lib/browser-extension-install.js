...
export async function installBrowserExtension({ currentPlatform = platform(), manifestDestinations } = {}) {
  ...
  await cp(join(packageRoot, "extension"), paths.extension, { recursive: true, force: true });
  ...
  const hostScript = join(packageRoot, "scripts", "browser-extension-host.mjs");
  ...
  return { ...paths, extensionId: metadata.extensionId, manifests: installedManifests };
}
