...
export function windowsRunCommand(nodePath, cliPath) {
  if (String(nodePath).includes('"') || String(cliPath).includes('"')) {
    throw new Error("Windows service paths cannot contain double quotes.");
  }
  return `"${nodePath}" "${cliPath}" serve`;
}

async function installWindowsTask(cliPath) {
  const valueName = "ChatGPTComputerControl";
  const runKey = "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run";
  const command = windowsRunCommand(process.execPath, cliPath);
  await run("reg.exe", ["ADD", runKey, "/v", valueName, "/t", "REG_SZ", "/d", command, "/f"]);
  ...
  child.unref();
  return { manager: "hkcu-run", name: valueName };
}
...
export async function removeService() {
  ...
  if (platform() === "win32") {
    await run("reg.exe", ["DELETE", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "ChatGPTComputerControl", "/f"], { capture: true }).catch(() => {});
