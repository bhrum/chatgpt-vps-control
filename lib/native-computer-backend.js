...
function helperPath() {
  ...
  if (platform() === "darwin") return join(base, "bin", "chatgpt-computer-helper");
...
async function invokeHelper(payload) {
  ...
  let output;
  if (os === "darwin") {
    output = await run(helperPath(), [], { input: request });
  } else if (os === "win32") {
...
function publicState(result, includeScreenshot) {
  const screenshot = includeScreenshot && result.screenshotBase64
    ? { mimeType: result.screenshotMimeType || "image/png", data: result.screenshotBase64 }
    : null;
  return {
    resolution: {
...
export async function nativeComputerUse(actions) {
  const result = await invokeHelper({ actions, includeScreenshot: true, includeWindows: false });
...
export async function nativeComputerElements(options = {}) {
  ...
  return {
    source: result.elementSource || nativeComputerBackendName(),
    application: result.elementApplication ?? null,
    elements: result.elements ?? [],
    message: result.elementMessage || `Returned ${(result.elements ?? []).length} accessibility elements.`,
  };
}

export async function nativeComputerElementAction({ elementId, action, value = "" }) {
  const result = await invokeHelper({
    actions: [],
    includeScreenshot: false,
    includeWindows: false,
    elementAction: { elementId, action, value },
  });
