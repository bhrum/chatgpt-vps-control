...
test("computer tools advertise cross-platform read/write capabilities", () => {
  ...
  assert.deepEqual(tools.map((tool) => tool.name), ["computer_environment", "computer_elements", "computer_element_action", "computer_state", "computer_use"]);
  ...
  assert.match(tools[1].description, /semantic accessibility tree/i);
  assert.match(tools[2].description, /snapshot/i);
  assert.match(tools[3].description, /local computer/i);
  assert.match(tools[3].description, /macOS/i);
  assert.match(tools[4].description, /platform-native/i);
});
